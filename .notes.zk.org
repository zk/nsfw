...
* <2018-04-10 Tue 13:08>


Styleguide inspo:

+ https://dribbble.com/shots/2341689-UI-Styleguide
+ https://dribbble.com/shots/2379228-Brand-Web-Refresh
+ https://dribbble.com/shots/3843565-Styleguide-2-Symbols-Styleguides-Freebie


* <2018-04-13 Fri 21:13>

Looking into the px vs rem thing, according to this bootstrap 4
discussion the main benefit is that rems causes the font size to scale
based on the user's default text size.

https://github.com/twbs/bootstrap/issues/19943


* <2018-04-14 Sat 11:18>

Ok, got this css thing split out pretty well, let's take it for a
spin.

The goal here is procedural design, in that a small set of input
parameters can produce a diverse set of asthetically pleasing designs
in a predictable and repeatable way.

+ Starting point:
  https://cl.ly/2l3c0e0O0i1j/Image%202018-04-14%20at%2011.24.06%20AM.png
+


* <2018-04-15 Sun 11:19>

Thinking about how to structure page dispatch. In some cases I want to
handle routing completely on the client.

The question I'm struggling with is whether or not to group everything
under 'views' (maybe wrong term), or split out into map entries.

Pros on views: all props localized, cons: views is wrong name.

Is there the case where you want the route to fire but no view render?
Put differently, should multiple routes go to same view? Yes,
differing logic should be handled in the handler. Multiple urls point
at same thing and logic is in the view and the handler.

Remove conditional from the handling flow, one less place to look for
bugs.

Why split views / handlers? Sometimes you want view no handler,
sometimes you want handler no view.


* <2018-04-16 Mon 10:11>

Modern page loading needs to be quick and SEO-aware. Splitting content
into above- and below-fold sections is the way to go. Better if you
can get the above-fold rendered statically.

Loading above-fold has it's own considerations

Form follows function, start simple -> complext

Keep them lightweight, otherwise you'll need loaders for your loaders.

Super low overhead, don't want to add too much unecessary wait
time. However, there's a floor here too. If your loading happens to
quickly or with too many elements it can't be followed by the user and
will be confusing or distasteful.


Most basic is fade-in:


Complex and content-specific https://dribbble.com/shots/4176034-Gali


Next: fade-in w/ element pop in
 https://dribbble.com/shots/4036134-The-Environmental-Law-Firm


+ Seeing the loading animation is super annoying, you want to see it
  once (or infrequently), to cache resources, esp on image / video
  heavy sites.
...
+ https://dribbble.com/shots/3901081-Loader-animation
+ More subtle:
  https://dribbble.com/shots/3230547-Logo-Plot-Load-Animation
+ https://dribbble.com/shots/4176034-Gali
+
...
So I need to solve two problems right now, vidbg not working in safari
(and firefox I'm assuming)


Man I gotta remember to put all this stuff in the styleguide

+
  https://rigor.com/blog/2016/01/optimizing-mp4-video-for-fast-streaming
+ Check resource loading errors in browsers.


* <2018-04-18 Wed 18:52>

Been thinking about further loader work. Couple of things:

+ Loader has to be initiated from the static page render. Otherwise
  slow app.js (which is probably the biggest asset anyway) will
  present a blank page only.
+ Initialization of js doesn't necessarily complete the loading
  lifecycle. JS may want to pull additional assets or perform
  long-running calculations.
+ Additionally, subsequent page loads where assets are cached
  shouldn't show the loading indicator. It's annoying to see it every
  time anyway. Can't think of any examples where


1. Loader rendered on backend. CSS animated.
2. JS attaches to

* <2018-05-10 Thu 10:29>

Writing API endpoints is way too scary at the moment, it's like I
don't have a good handle on a few things:

+ What are all the endpoints?
+ Per endpoint, what are the expected preconditions?
+ What are all the error states per endpoint
+ What are all the side effects and return values for the endpoint?
+ How do I quickly access all this information, including drill-down
  and search?
+ Are they "hooked up" correctly? Meaning transit encoding / decoding,
  routes, etc.

In general there's one success state and many error states. Error
states have to do with validating input data, availability and success
of calling external resources (APIs, databases, files).

The tricky part is that some of these checks and validations depend on
the success or failure of previous checks.


Ok, let's answer these questions.

Uh-oh, I feel a rabbit hole coming on.

Ok, so where are all the endpoints? For GI endpoints they're all int
he handler map. There's a name (key) and there's a handler with a
possible docstring. Since it's clojure we can presumably grab all this
through introspection.

The linear thing isn't enough, its' too easy to lose context. If I'm
adding another window it's got to be comprehensive.

For all endpoints is order important? Grouping is imortant, I guess
order is important too.


Ooh, interesting, already paying dividens

https://cl.ly/3o3f3i1d2e3L/Screen%20Shot%202018-05-10%20at%2011.35.33%20AM.png

Just noticed the shape of the return value of the success case is not
the same as the return value shape of the error cases.


* <2018-05-24 Thu 10:50>

Decided to switch off of onesignal due to the data privacy stuff. They
basically sell all info about your users (which they are transparent
about) and I'm not super comfortable about that.

Besides it's $1 for 1M push notifications on SNS so why not. There's a
bit more set up.

To send a push you need:
